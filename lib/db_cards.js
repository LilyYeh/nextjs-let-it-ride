...
export async function getCards(socketId, limit) {
	const querySQL = `SELECT cardId, number, type, CONCAT(number,type) imgName 
					  FROM cards 
...
export async function foldCards(socketId='') {
	let updateSQL = `UPDATE cards SET isDeleted=1, socketId=null WHERE socketId='${socketId}'`;
	if(socketId == ''){
		updateSQL = `UPDATE cards SET isDeleted=1, socketId=null WHERE socketId is not null`;
	}
