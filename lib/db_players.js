...
export async function goaling(socketId, bets, baseMoney, baseMyMoney) {
	...
	if(baseAllMoney - totalPlayersMoney <= 0){
		const updateSQL = `UPDATE banker SET money=0 WHERE deletedTime is null`;
		await query({ query: updateSQL });
	}*/
...
export async function setKeepGoing(baseMoney) {
	...
	//const querySQL = `SELECT money FROM banker WHERE deletedTime is null`;
	//const banker = await query({ query: querySQL });
	//if(banker[0].money <= 0){
		//const totalPlayers = await countPlayers();
		//const updateSQL1 = `UPDATE banker SET money=${baseMoney*totalPlayers} WHERE deletedTime is null`;
		//await query({ query: updateSQL1 });
